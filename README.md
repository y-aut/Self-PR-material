...
研究テーマ: 「降雨データを用いた SNS ユーザーの居住地推定」

卒業研究で作成したプログラム等を以下にまとめます．

### 解析雨量データの変換ツール (C++)

研究では過去 5 年間の全国の解析雨量データが収録された DVD-ROM を使用した．このデータは，1km 四方のメッシュで 30 分間隔で記録されており，緯線に並行な方向でランレングス圧縮されている．

このデータについての操作を行う CUI アプリケーションを作成した．
コマンドの一覧はレポジトリの main.cpp 中にある，使用方法を表示する [show_usage 関数](https://github.com/y-aut/decoder/blob/master/src/main.cpp#L12)に詳しく載っている．

例えば，データファイルから以下のようなビットマップを生成できる．

<div style="display: flex; gap: 10px; margin: 10px 0;">
  <img src="./img/thesis/raw.png" alt="データ例" width="250pt">
  <img src="./img/thesis/syn.png" alt="データ例" width="250pt">
  <img src="./img/thesis/sample.png" alt="データ例" width="250pt">
</div>

↑ 左から，(a) ある時刻の解析雨量データを段階別に色をつけて表示したもの．(b) 複数の時刻について合成したもの．(c) ユーザーの居住地の推定結果．

図 (b) は解析雨量データを複数の時刻について合成したものであるが，このような合成データをファイルとして出力するコマンドでは，ランレングス圧縮を施されたデータのまま各資料点を走査し，合成した結果もまたランレングス圧縮されたデータとして出力するようにしている．これにより，時間計算量・空間計算量をともに削減することができる．

他にも，ユーザーの居住確率が高い地点を，複数のオプションを指定してランキング形式で出力するコマンドなどを実装している．

### 機械学習スクリプト (Python)

研究では，投稿本文が実際に雨が降っていることを表すものかどうかを判定する機械学習モデルを訓練して使用した．訓練には実際の降雨情報をもとにした教師データを用い，PU Learning という特殊な学習手法を使用した．

自然言語処理用のライブラリ Hugging Face や PyTorch を用いて機械学習を行った．Sentence BERT の学習済みモデルの出力を利用し，学習データに適切に重みを付けて教師ラベルの値をもとに訓練を行った．

学習後のモデルの性能の評価手法の一つとして，居住地の判明しているテストユーザーにおいて，過去の投稿から実際に居住地を推定して精度を表すスコアを算出する処理を，各学習後に自動で行うようにした．

### Twitter クローラ (Python)

Twitter API を定期的に呼び出してユーザー・投稿データを収集するクローラを作成した．

Systemd を使用してサーバーに常駐させており，ログローテーション等の機能も実装している．

### その他の Python スクリプト

他にも，

- 取得したユーザーのプロフィールにある居住地情報を実際の市区町村にパースするスクリプト
- 投稿に対して，その時刻の降雨情報を上述の CUI アプリケーションにより取得し，雨が降っているかどうかでラベリングするスクリプト

など，様々な処理を行うスクリプトを記述した．
